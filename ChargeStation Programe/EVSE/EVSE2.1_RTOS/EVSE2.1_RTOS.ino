...
void setup() {
  ...
  if (pzem.current() <= limitCurrent) {
    if (pzem.voltage() >= MinVoltage && pzem.voltage() <= MaxVoltage) {
      ...
      Serial.println("Warning! The system might face over/under voltage from grid");
      delay(1000);
    }
  } else {
    Serial.println("Warning! The system might face over current from grid");
  }
  
  // // Start RTOS tasks
  // while (pzem.current() <= limitCurrent) {
  //   if (pzem.current() <= limitCurrent) {
  //     while (pzem.voltage() >= MinVoltage && pzem.voltage() <= MaxVoltage) {
  //       if (pzem.voltage() >= MinVoltage && pzem.voltage() <= MaxVoltage) {
  //         xTaskCreatePinnedToCore(WifiConnectTask, "WifiConnectTask", 4096, NULL, 1, NULL, 0);
  //         xTaskCreatePinnedToCore(controlPilotTask, "ControlPilotTask", 4096, NULL, 1, NULL, 1);
  //         xTaskCreatePinnedToCore(currentDetectionTask, "CurrentDetection", 2048, NULL, 1, NULL, 1);
