...
app.get('/checking', (req, res) => {
  ...
  if (stationID && !userID) {
    db.query('SELECT connection FROM stations WHERE stationID = ?', [stationID], (err, results) => {
        ...
        if (results.length === 0) {
            return res.status(404).json({ message: 'Station not found', connectionAvailable: null });
        }

        const connectionStatus = results[0].connection;

        if (connectionStatus === 'Available') {
            return res.status(200).json({ connectionAvailable: true });
        } else {
            return res.status(200).json({ connectionAvailable: false, currentConnection: connectionStatus });
        }
    });
    return;
}
...
if (!stationID && userID) {
  db.query('SELECT stationID FROM stations WHERE connection = ?', [userID], (err, results) => {
      if (err) {
      ...
      if (results.length === 0) {
          console.log('No stationID found for the given userID.');
          return res.status(404).json({ message: 'Station not found' });
      }

      // Return stationID directly
      const stationID = results[0].stationID; // Ensure 'stationID' matches DB column name
      console.log('StationID from DB:', stationID);

      return res.status(200).json({ stationID: stationID });
  });
  return;
}
