...
app.post('/updatenotification', async (req, res) => {
  const { userID, message } = req.body; // Get ID and new message from request body
  ...
  const query = 'UPDATE Notification SET message = ? WHERE userID = ?';

  db.query(query, [message, userID], (err, result) => {
    if (err) {
...
app.get('/checking', (req, res) => {
  const { stationID, chargeStatuse, userID } = req.query;

  if (!stationID) {
      return res.status(400).json({ error: 'stationID is required' });
  }

  if (stationID && !chargeStatuse && !userID) {
  ...
  if (stationID && chargeStatuse && userID) {
      ...
      if (chargeStatuse && userID) {
          updates.push('chargeProcess = ?');
          values.push(chargeStatuse);
      }

      values.push(stationID);

      const query = `UPDATE stations SET ${updates.join(', ')} WHERE stationID = ?`;
      db.query(query, values, (err, result) => {
          if (err) return res.status(500).json({ error: 'Failed to update station data' });
          if (result.affectedRows === 0) return res.status(404).json({ error: 'Station not found' });

          if (chargeStatuse && userID) {
              db.query('SELECT targetPower, chargePower, payment, cost FROM stations WHERE stationID = ?', [stationID], async (err, status) => {
                  if (err) return res.status(500).json({ error: 'Database error occurred' });
                  if (status.length === 0) return res.status(404).json({ error: 'Station not found' });

                  const { payment, chargePower, cost, targetPower } = status[0];
                  const power = targetPower * chargePower / 100;

                  // Check if power is NaN
                  if (isNaN(power)) {
                      return res.status(400).json({ error: 'Invalid power value' });
                  }

                  if (chargeStatuse === 'stop' || chargeStatuse === 'error') {
                      const refund = cost * (100 - chargePower) / 100;

                      // Check if refund is NaN
                      if (isNaN(refund)) {
                          return res.status(400).json({ error: 'Invalid refund value' });
                      }

                      if (payment === 'paid') {
                          db.query('SELECT wallet FROM users WHERE userID = ?', [userID], async (err, user) => {
                              if (err) return res.status(500).json({ error: 'Database error occurred' });
                              if (user.length === 0) return res.status(404).json({ error: 'User not found' });

                              const updatedWallet = parseFloat(user[0].wallet) + refund;
                              db.query('UPDATE users SET wallet = ? WHERE userID = ?', [updatedWallet, userID], async (err, userUpdate) => {
                                  if (err || userUpdate.affectedRows === 0) return res.status(500).json({ error: 'Failed to update wallet' });

                                  const message = chargeStatuse === 'stop'
                                      ? `You have successfully stopped your charging session. A refund of $${refund} has been processed. We appreciate your use of our service and thank you for your understanding.`
                                      : `Unfortunately, an issue occurred during your charging session. We apologize for the inconvenience. A refund of $${refund} has been processed for you. Thank you for your understanding`;
                                  await insertNotification(userID, message);
                                  await insertHistoricalData(userID, stationID, power, refund);
                                  return res.status(200).json({ message: `Charging status updated: ${chargeStatuse}` });
                              });
                          });
                      }
                  } else if (payment === 'paid' && chargeStatuse === 'complete') {
                      // For 'complete' charge status, no refund, just a success message
                      ...
                      await insertNotification(userID, message);
                      await insertHistoricalData(userID, stationID, power, cost); // Log the power used and the cost
                      return res.status(200).json({ message: `Charging complete. Power used: ${power}kWh` });
                  }
              });
          } else {
              return res.status(200).json({ message: 'Station data updated successfully' });
          }
      });
      return;
  }
...
app.get('/payment', (req, res) => {
  // Access userID and stationID from query parameters
  const { userID, stationID } = req.query;

  // Validate that userID and stationID are provided
  if (!userID || !stationID) {
    return res.status(400).json({ error: 'userID and stationID are required' });
  }

  // Step 1: Get the station's payment status
  db.query('SELECT payment FROM stations WHERE stationID = ?', [stationID], (err, status) => {
    if (err) {
      return res.status(500).json({ error: 'Database error occurred' });
    }

    if (status.length === 0) {
      // If no station found with that stationID
      return res.status(404).json({ error: 'Station not found' });
    }

    let PaymentState = status[0].payment; // Payment state should be treated as a string (e.g., 'paid' or 'unpaid')

    // Step 2: Check if the station is already paid
    if (PaymentState === 'paid') {
      return res.status(400).json({ error: 'Already Paid' });
    }

    // Step 3: Get the user's wallet balance
    db.query('SELECT wallet FROM users WHERE userID = ?', [userID], (err, users) => {
      ...
      if (users.length === 0) {
        // If no user found with that userID
        return res.status(404).json({ error: 'User not found' });
      }

      let money = parseFloat(users[0].wallet); // Ensure the wallet is treated as a float

      // Step 4: Get the station's cost
      db.query('SELECT cost FROM stations WHERE stationID = ?', [stationID], (err, stations) => {
        if (err) {
          return res.status(500).json({ error: 'Database error occurred' });
        }
        ...
        // Step 5: Check if the user has enough money
        if (money >= cost) {
          const total = money - cost;

          // Step 6: Update the user's wallet balance
          db.query('UPDATE users SET wallet = ? WHERE userID = ?', [total, userID], (err, userUpdate) => {
            ...
            const connectionStatuse = 'Unavailable';

            // Step 7: Update the station's payment status
