document.addEventListener('DOMContentLoaded', async function () {
        ...
        return new Promise((resolve, reject) => {
            ...
            confirmButton.addEventListener('click', async function (e) {
                ...
                try {
                    ...
                    const response = await fetch(`http://localhost:8081/confirmuser?userID=${userID}&password=${password}`);
    ...
    function checkAndSaveStationID() {
        ...
        if (scannedStationID) {
            ...
            if (storedStationID === scannedStationID) {
                alert('StationID matches the stored value!');
            } else {
                const confirmSave = confirm(
                `StationID from the link (${scannedStationID}) does not match the stored value. Do you want to update it?`
                );
        
                if (confirmSave) {
                localStorage.setItem('stationID', scannedStationID);
                alert('StationID updated successfully!');
                } else {
                alert('StationID not updated.');
                }
            }
        } else {
            alert('No StationID found in the URL.');
        }
    }
                ...
                nextButton.addEventListener('click', function (e) {
                    ...
                    openConfirmForm(document.getElementById('BankForm')).then((confirmed) => {
                        if (confirmed) {
                            ...
                            const bankInfoURL = `http://localhost:8081/bankinfo?userID=${userID}&BankHolder=${BankHolder}&BankNumber=${BankNumber}`;
    ...
    async function populateData() {
        ...
        if (stationID) {
            try {
                const response = await fetch(`http://localhost:8081/requestData?stationID=${stationID}`);
                ...
                if(stationData.chargeProcess == "stop" || stationData.chargeProcess == "error" || stationData.chargeProcess == "N/A"){
                    localStorage.removeItem('stationID');                    
                }
                if (stationData.chargeProcess.trim().toLowerCase() === "complete") {
                    const userID = localStorage.getItem('userID');
                    const stationID = localStorage.getItem('stationID');
                    
                    // Check if userID or stationID is missing
                    if (!userID || !stationID) {
                        console.error('Missing userID or stationID');
                        return; // Exit if either value is missing
                    }
            
                    // Construct the URL for the API request
                    const url = `http://localhost:8081/checking?stationID=${stationID}&chargeStatuse=complete&userID=${userID}`;
                    ...
                    fetch(url)
                        ...
                        .then(data => {
                            console.log('Response Data:', data);  // Log the data from the server
                            localStorage.removeItem('stationID');
                        })
                        .catch(error => {
        ...
        if (userID) {
            try {
                const response = await fetch(`http://localhost:8081/requestData?userID=${userID}`);
    ...
    async function Historical_Data() {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8081/recordeData?userID=${userID}`);
            if (!response.ok){
                return;
            }
            ...
            if (dates && Array.isArray(dates)) {
                ...
                if (CurrentSize !== lastSize || !arraysEqual(dates, lastData)) {
                    ...
                    for (let i = 0; i < dates.length; i++) {
                        ...
                        const Dateresponse = await fetch(`http://localhost:8081/recordeData?userID=${userID}&date=${currentDate}`);
    ...
    async function Notification_Data() {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8081/getnotifications?userID=${userID}`);
            if (!response.ok) throw new Error('Failed to fetch user data from the API');
                ...
                notifications.forEach(notification => {
                    ...
                    DeleteNotification.addEventListener('click', async () => {
                        try {
                            ...
                            const response = await fetch(`http://localhost:8081/deletenotification?userID=${userID}&ID=${ID}`, {
                                method: 'DELETE',
                            });
    ...
    document.getElementById('Discharge').addEventListener('click', function() {
        ...
        document.getElementById('confirmYes').addEventListener('click', function() {
            ...
            const userID = localStorage.getItem('userID');
            const url = `http://localhost:8081/checking?stationID=${stationID}&chargeStatuse=stop&userID=${userID}`;
    ...
    function makePayment(userID, stationID) {
        const url = `http://localhost:8081/payment?userID=${userID}&stationID=${stationID}`;
        ...
        fetch(url)
            ...
            .then(data => {
                if (data && data.message) {
                    ...
                    alert(`Payment failed for User ID: ${userID}, Station ID: ${stationID}. Invalid server response.`);
                }
            })
            .catch(error => {
    ...
    function checkStationID(stationID) {
        const url = `http://localhost:8081/checking?stationID=${stationID}`;
        return fetch(url)
            .then(response => {
                if (!response.ok) {
            ...
            .then(isValid => {
                return isValid;  // Return the result directly
            })
            .catch(error => {
    ...
    // Handle QR code stationID and payment process
    function handleStationID(stationID) {
        const storedStationID = localStorage.getItem('stationID');
        
        if (storedStationID === stationID) {
            console.log('Station ID already validated. Proceeding to payment.');
            makePayment(localStorage.getItem('userID'), stationID);
        } else {
            checkStationID(stationID)
                .then(isValid => {
                    if (isValid) {
                        console.log('Station ID validated. Proceeding to payment.');
                        makePayment(localStorage.getItem('userID'), stationID);
                    } else {
                        alert('Invalid station ID. Payment cannot be processed.');
                    }
                })
                .catch(error => {
