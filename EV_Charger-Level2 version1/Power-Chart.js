        ...
        try {
            ...
            if (Array.isArray(years)) {
                ...
                for (let month = 1; month <= 12; month++) {
                    const res = await fetch(`http://localhost:8081/recordeData?userID=${userID}&month=${month}&year=${latestYear}`);
                    if (res.ok) {
        ...
        try {
            ...
            if (Array.isArray(years)) {
                ...
                for (const year of years) {
                    const res = await fetch(`http://localhost:8081/recordeData?userID=${userID}&year=${year}`);
                    if (res.ok) {
                        const data = await res.json();
                        const value = parseFloat(data.value); // Ensure the value is a number
                        if (!isNaN(value)) {
