document.addEventListener('DOMContentLoaded', async function () {
        ...
        if (userID && stationID) {
            try {
                ...
                const [userResponse, stationResponse] = await Promise.all([
                    fetch(`http://localhost:8081/requestData?userID=${userID}`),
                    fetch(`http://localhost:8081/requestData?stationID=${stationID}`)
                ]);
    ...
    document.getElementById('Discharge').addEventListener('click', function() {
        ...
        document.getElementById('confirmYes').addEventListener('click', function() {
            ...
            const url = `http://localhost:8081/stationProcess?stationID=${stationID}&chargeStatuse=stop`;
    ...
    function makePayment(stationID) {
        const url = `http://localhost:8081/payment?stationID=${stationID}`;
    ...
    function checkStationID(stationID) {
        ...
        return fetch(url)
            ...
            .then(data => {
                // Assuming 'connectionAvailable' is the field indicating if the station is available
                return data.connectionAvailable || false;  // Return true/false based on availability
            })
            .catch(error => {
    ...
    function connectStationID(stationID, userID) {
        const url = `http://localhost:8081/checking?stationID=${stationID}&userID=${userID}`;
        return fetch(url)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
            ...
            .then(isAvailable => {
                if (isAvailable) {
                    ...
                    connectStationID(stationID, userID)
                        .then(success => {
                            if (success) {
                                const url = `http://localhost:8081/stationProcess?stationID=${stationID}&chargeStatuse=charge`;
                                ...
                                fetch(url)
                                    ...
                                    .then(data => {
                                        // Successfully connected to the station, hide the Charge element and reload
